...
class ILI9341:
    ...
    def erase(self, x=0, y=0):
        self.fill_rectangle(0, 0, self.width - x, self.height - y)
    ...
    def chars(self, str, x, y):
        ...
        for ch in str:
            ...
            glyph_nbytes = (char_w + 7) // 8  # ← number of bytes per row for this char
    ...
    def print(self, text): #does word wrap, leaves self._x unchanged
        ...
        char_h = self._font.height() + 2
        char_w = self._font.max_width() + 2
        lines = text.split('\n')
        for line in lines:
            words = line.split(' ')
            for word in words:
